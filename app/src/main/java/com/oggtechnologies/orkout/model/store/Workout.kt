...
val SetDataField.name get() = when (this) {
...
val SetDataField.unit get() = when (this) {
...
val SetDataField.key get() = when (this) {
    is SetDataField.Weight -> "weight"
    is SetDataField.Reps -> "reps"
    is SetDataField.Time -> "time"
    is SetDataField.Distance -> "distance"
...
val Exercise.template: ExerciseTemplate? get() = appStore.state.exerciseTemplates.find { it.id == templateId }
val Exercise.name: String get() = template?.name ?: "Unknown exercise"
...
fun doStartWorkout(): Action {
    ...
    return SetActiveWorkout(workout)
}

data class SetActiveWorkout(val workout: Workout?) : Action
